...
@router.get("/all_view_authorsBook", response_model=list[ViewAuthorsBookSchema])
async def get_all_authorsBook() -> list[ViewAuthorsBookSchema]:
    async with database.session() as session:
        await authorsBook_repo.check_connection(session=session)
        all_authorsBook = await authorsBook_repo.get_all_view_authorsBooks(session=session)
